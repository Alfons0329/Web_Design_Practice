...
function draw()
{
  ...
  {
    var angle=radians(i); //a conversion
    //Polar coordinate conversion (Polar to x,y in Cartesian)
    var x=cx+clockRadius*0.9*(cos(angle));
    var y=cy+clockRadius*0.9*(sin(angle));
    if(i%30==0)
    {
      stroke(255,255,0);
      strokeWeight(15);
  ...
  //angle from 3'oclock since there is 0PI , and use map functinon to convert R(var)->R(rad in polar coordinate)
  ...
  var sec_in_rad = map(second(), 0, 60, 0, TWO_PI) - HALF_PI;
  var min_in_rad = map(minute() + norm(second(), 0, 60), 0, 60, 0, TWO_PI) - HALF_PI;
  var hour_in_rad = map(hour() + norm(minute(), 0, 60), 0, 24, 0, TWO_PI*2) - HALF_PI;
